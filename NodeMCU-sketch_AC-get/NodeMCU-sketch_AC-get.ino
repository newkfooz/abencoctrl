...
int Vmax = 0;                                   // variables for electric current reading:
int lettArray[100];
const int refReadVal = 521;
float Irms;
float kWatt;
const float maxCurrent = 13;
const float notifyCurrent = 10;
...
void ControlsByMessage(String tpc, String msg){
  ...
  if (tpc == "/ABI_TIR/REQ"){ 
    ...
    //#### Set relay status ####//
    if (reqType == "SET" && reqDetail == "relay"){
        if (reqValue == "ON"){
    ...
    //#### Info request ####//
        ...
        if (reqDetail == "relay" && reqValue == "STATUS"){
        ...
        }else if (reqDetail == "current" && reqValue == "STATUS"){
                getCurrent();
                sendCurrent(reqId);
        }else{
                String errorValue = "ERROR";
...
void getCurrent(){
      ...
      for (int i=0; i<100; i++){
          lettArray[i] = analogRead(A0);
          Vmax = (lettArray[i]>Vmax)?lettArray[i]:Vmax;
      }
      
      Irms = (((float)Vmax - (float)refReadVal)*2000)/(1024*20*sqrt(2));
...
////_______ Loop read Current value and EVENT MQTT _______///

void readLoopCurrent(){
      getCurrent();
      if (Irms >= notifyCurrent && Irms < maxCurrent){
